...
while (my $file = readdir D) { 
    ...
    while (<F>) {
        ...
        if (m/^0kimura/) {
            ...
            foreach my $r (@a) {
                $r =~ s/\?//g;
                # Specific class change requested by Arian
                $r =~ s#DNA/Chompy#DNA#;
